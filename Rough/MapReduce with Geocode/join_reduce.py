    ...
    # Block for all inputs after the 1st one
        ...
        if common == currentCommonKey:
            ...
            if (re.search(r'Trips', currentRest[0]) and re.search(r'Fares', rest[0])):
                # Get coordinates 
                pickup_long = float(currentRest[7])
                pickup_lat = float(currentRest[8])
                drop_long = float(currentRest[9])
                drop_lat = float(currentRest[10])
                ...
                pickup_long = float(rest[7])
                pickup_lat = float(rest[8])
                drop_long = float(rest[9])
                drop_lat = float(rest[10])
                if check(pickup_long, pickup_lat):
                	pick_boro = check(pickup_long, pickup_lat)
                else:
                	pick_boro = 'error'
